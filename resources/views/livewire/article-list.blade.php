<div>
    <div class="mb-6 flex items-center justify-between">
        <h1 class="text-2xl font-bold text-gray-900">Latest Articles</h1>
    ...
    <div class="mb-6 grid grid-cols-1 gap-3 rounded-lg border border-gray-200 bg-white p-4 sm:grid-cols-2 lg:grid-cols-5">
        <div class="lg:col-span-2">
            ...
            <input
                ...
                class="mt-1 block w-full rounded-md border border-gray-300 px-3 py-1.5 text-sm focus:border-blue-500 focus:outline-none focus:ring-1 focus:ring-blue-500"
            >
        </div>
    ...
    >
        @if ($this->articles->isEmpty())
            <div class="rounded-lg border border-dashed border-gray-300 bg-white px-6 py-16 text-center">
                @if ($search !== '' || $category !== '' || $source !== '' || $from !== '' || $to !== '')
                    <p class="text-lg font-medium text-gray-900">No articles match your filters</p>
                    <p class="mt-2 text-sm text-gray-500">
                        Try widening your search, or
                        <button type="button" wire:click="resetFilters" class="font-medium text-blue-700 hover:underline">clear all filters</button>.
                    </p>
                @else
        ...
        @else
            <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-3">
                @foreach ($this->articles as $article)
                    <article wire:key="article-{{ $article->id }}" class="flex flex-col overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm transition hover:shadow-md">
                                    ...
                                    @auth
                                        @if (in_array($article->id, $this->bookmarkedIds, true))
                                            <button
                                                ...
                                                class="inline-flex items-center gap-1 rounded-md border border-blue-200 bg-blue-50 px-2.5 py-1 text-xs font-medium text-blue-700 hover:bg-blue-100 disabled:opacity-50"
                                            >
                                        ...
                                        @else
                                            <button
                                                ...
                                                wire:loading.attr="disabled"
                                                wire:target="toggleBookmark({{ $article->id }})"
                                                class="inline-flex items-center gap-1 rounded-md border border-gray-200 bg-white px-2.5 py-1 text-xs font-medium text-gray-600 hover:bg-gray-50 disabled:opacity-50"
                                            >
